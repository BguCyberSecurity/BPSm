...
[
  ...
  for (var col = 2; col >= 0; col--) {
    permutations.forEach(function(perm) {
      var first = new move(perm[0], col);
      var second = new move(perm[1], col);
      var third = new move(perm[2], col);
      var btname = genWinBTName(winEvent, first, second, third);
      var func = genDetectWinBT(first, second, third, winEvent);
      bpjs.registerBThread(btname, func);
    });
  ...
  /*    reverse diagonal*/
  permutations.forEach(function(perm) {
    var first = new move(perm[0], 2 - perm[0]);
    var second = new move(perm[1], 2 - perm[1]);
    var third = new move(perm[2], 2 - perm[2]);
    var btname = genWinBTName(winEvent, first, second, third);
    var func = genDetectWinBT(first, second, third, winEvent);
    bpjs.registerBThread(btname, func);
  });
